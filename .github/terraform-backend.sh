...
# print usage and exit if no resource group name
if [[ -z "$resource_group_name" ]]; then
    usage
fi

# print usage and exit if no storage account name
...
echo "Checking for existence of $storage_account_name storage account..."
echo
...
if [[ "$container_exists" = "true" ]]; then
    echo "Storage account container (terraform-state) exists!"
    echo
else
    echo "Storage account container (terraform-state) does not exist."
    echo "Attempting to create..."
